 ...
 */
@Repository("stockPrice60MinDao")
public class StockPrice60MinDaoImpl extends BaseDaoImpl<Stockprice60Min> implements StockPrice60MinDao {
    @Override
    public List<Object[]> selectListByStockName(String stockName, int maxSize) {
        SQLQuery sqlQuery = this.getSession().createSQLQuery("select s.Date,s.Open,s.Close,s.Low,s.High  from data_analysis.stockprice_60min as s where s.StockName ='"+stockName+"'");
